;;;; cyco/Local/Emu/Procussion/percussion2.lisp
;;;; 2013.04.22
;;;; Instruments
;;;;    percussion2
;;;;        multidrum2
...
(defmap --percussion2-drum-map '((conga   . 36)
				(conga2  . 61)
				(conga3  . 37)
				(conga4  . 62)
				(conga5  . 38)
...
(defmap --percussion2-shaker-map '((shaker  . 43)
				  (shaker2 . 44)
				  (shaker3 . 42)
				  (shaker4 . 45)
				  (noise   . 52)
...
(defmap --percussion2-cow-map '((palstic   . 43)
			       (plastic2  . 44)
			       (plastic3  . 42)
			       (plastic4  . 45)
			       (blk       . 73)
...
(defmap --percussion2-clave-map '((x  . 98)
				 (x2 . 49)
				 (x3 . 59)
				 (x4 . 48)
				 (x5 . 51)))

(defmap --percussion2-guiro-map '((x  . 101)
				 (x2 . 102)
				 (x3 . 67)
				 (x4 . 68)
				 (x5 . 66)
...
(defmap --percussion2-amazon-map '((x  . 85)
				  (x2 . 86)
				  (x3 . 84)
				  (x4 . 87)))
...
(defun percussion2 (&optional (parent pro3))
  (remove-children parent)
  (set-value parent :program (procussion-program-map 'percussion2))
  (let ((kinst (instrument 'percussion2
			   :parent parent)))
    (param multidrum2 (instrument 'multidrum2
				  :parent kinst
				  :keymap #'--percussion2-drum-map))
    (param shaker2 (instrument 'shaker2
			       :parent kinst
			       :keymap #'--percussion2-shaker-map))
    (param cow2 (instrument 'cow2
			    :parent kinst
			    :keymap #'--percussion2-cow-map))
    (param clave2 (instrument 'clave2
			      :parent kinst
			      :keymap #'--percussion2-clave-map))
    (param guiro2 (instrument 'guiro2
			      :parent kinst
			      :keymap #'--percussion2-guiro-map))
    (param amazon2 (instrument 'amazon2
			       :parent kinst
			       :keymap #'--percussion2-amazon-map))
    (setf percussion2 kinst)
    kinst))
